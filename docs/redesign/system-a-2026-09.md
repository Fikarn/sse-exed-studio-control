...
| Region        | What it is                                                                                                 | Studio |
| ------------- | ---------------------------------------------------------------------------------------------------------- | -----: |
| Header        | the shell: crest, product, four tabs, three subsystem lamps, latches (Solo, Scene unsaved), the clock      |     56 |
| Cluster       | plate, left: the **state display**, the take-time keys, the lists, the standing actions (bottom)           |    424 |
| State display | a black display, fixed height: lamp + word, the engine's sentence, the way-out key; nothing below it moves |    180 |
| Bay           | the recessed floor the workspace's picture sits on: strips, the plot, the timeline, the step screen        |   rest |
| Plate         | plate, right: the selection (strip, fixture, task) or Support                                              |    416 |
| Footer        | telemetry as `Label value` items and one action key (no key hints since the new pages program's Slice 3)   |     40 |

The operator ruled on 2026-09-07 that `2560×1440` is the only surface that matters, and on 2026-09-26 that Studio Control always runs there, fullscreen on Windows (the new pages program's D22). The live-minimum and utility layouts the mocks also draw are a record: the app has one layout since that program's Slice SW. Density never comes from the type; the sizes stay.
...
|     +3 | Drawer  | plate recipe + `0 12px 32px black/60 %`                                                      | dialogs, toasts (to be drawn)                                          |
...
- **Bay pictures**: `StagePlot` (SVG at rendered size, fixtures, beams tinted by CCT, camera and subject marks), `Timeline` (lanes, cards at start with duration bars, second row on overlap, edge cards hang left, thinner hour labels below 90 px/h, now marker, unscheduled tray), `StepScreen` (content height).
...
Sentence case for commands (verb + object), the deck's words on key caps (mono, uppercase), the engine's words for states. Name the hardware: TotalMix, the UFX III, the bridge, the deck, Companion, the rig. Never engine, backend, transport, IPC in operator copy; "snapshot" only for the Console's scene primitive; "Engine log" stays as the feature's name until the engine copy pass renames it. Numbers carry sign and unit (`-3.8 dB`, `+2.1 dB`, `32 dB`, `3200 K`, `76 %`, `18:24`, `85 min`, `1h 18m`, `U1`, `192.168.1.80`). Every state sentence says what happened and what to do; the way out is a key in the same display. No surface shows a key, a key glyph or a key hint, and every action has a control on the screen (the new pages program's Slice 3, D6, 2026-09-26): the keyboard does only what it does in any program — Tab, Enter or Space on the focused control, typing, the arrows on a focused slider or list, Esc on a dialog, a popup or an armed key. `scripts/check-no-shortcuts.mjs` holds it at 0.

## 10. Measures (the gates the plan turns into tests)

Per board, per theme, at 2560×1440: no page scroll; type floor 12 px and ≤ 8 sizes, families ⊆ {Inter, JetBrains Mono}; pixel-sampled contrast ≥ 4.5:1 for text (≥ 3:1 large), locked controls at 55 % exempt; every enabled target ≥ 24 px, take-time ≥ 28 px (32 in the studio); radii ⊆ {4, 8, 12, pill}; shadows with no negative offset, blur > 8 px only on lit elements or the drawer; gradients only on plates, keys, caps, meters and the CCT track; 0 running animations at idle; chrome heights within ±2 px of §2; state display at the same place on every workspace; copy scan at 0 forbidden words. Current (Slice 11, 2026-09-09): 81 boards — 27 fixtures in three themes — with every measure on this line met on every one of them: floor 12 px, at most 8 sizes, 0 contrast fails, 0 small targets, 0 radii off policy, 0 idle animations, 0 forbidden words, and no target off the viewport at all — the measure is strict, and a relaxed one that would have excused a target with a scrollable ancestor was tried in Slice 11 and reverted. Since 2026-09-24 (new pages program, Slice 1): 66 boards — 22 fixtures in three themes; Planning's five fixtures left with the workspace. Since 2026-09-26 (Slice SW): 63 boards — 21 fixtures; `audio-1920-fallback` left with the other sizes, and contrast is sampled on the studio workstation only (Windows), not on CI's Linux runner.

## 11. Decisions carried forward (for the plan's decision table)

The cluster rule replaces the workspace top bar · the state display is fixed-height and first · material v2 with blooms on lit states · a 12 px floor and a nine-step scale · selection is neutral, amber is engaged, blue is the fourth hue · 48 V on is a red lamp and word, not a red fill · locked is a dashed outline at 55 % with the reason in reach · scene recall is one press, saving over a scene is two · Planning's `ON TIME` is derived until the engine owns it · Setup's step screen is content-height · the deck's short names appear on 84 px strips · theme names stay Studio, Graphite, Bone.
