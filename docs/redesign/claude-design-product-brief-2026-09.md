...
- The only surface: `2560×1440` logical pixels, fullscreen, fixed. No other size is designed for (the operator's ruling of 2026-09-26).
...
- **Continuous.** Faders, gain, intensity, colour temperature, grand master: drag; on the focused control the arrow keys step, Home and End go to the ends, and Page Up and Page Down take a larger step where the control has one — no modifier key changes a step (the new pages program's Slice 3, decision 9); Enter or a double-click opens typed entry where the control has one, which offers "Reset to <default>" where it has a default (the strip faders also keep right-click › Reset to unity); commits throttled while dragging, final on release.
...
2. After one is chosen: the Console in its blocked states (not verified, assumed, offline, armed), then Lighting, Planning and Setup in the same language, then the three themes.
