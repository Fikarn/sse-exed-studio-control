...
| **Jump-around warning**        | Clicking a future step opens "Skip ahead? Preceding steps haven't been confirmed." — since the new pages program's Slice 3 the design system's confirm dialog: it takes focus, and Esc or Cancel close it without skipping. | n/a                                                                                                                                                                 | User clicks a step header > `indexOf(commissioningStage)`.                                  |
...
**Removed on 2026-09-26 (the new pages program, Slice 3, decision D6).** Setup binds no key of its own and shows no key hint: Runner and Support are keys on the page, Tab moves focus again, Enter presses only the focused control, and the Map's page tabs and deck controls are clicked. `docs/plans/new-pages-2026-09-s3-inventory.md` names each key's on-screen twin. The table is kept as the record of what there was.
