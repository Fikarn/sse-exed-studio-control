...
- `main` branch protection applies to administrators and requires stale-review dismissal, code-owner review, last-push approval, one approving review, linear history, conversation resolution, and strict required status checks.
...
| R4 Branch hygiene           | verified | Deleted 25 merged local branches, deleted 12 stale remote branches, pruned the already-removed Dependabot branch, and deleted three local squash-merged PR heads. After the follow-up pass, local branches are `main` and `codex/repo-professionalization-followup`; remote branches are `origin/main` plus the active PR branch.                                             |
...
| R16 GitHub governance       | verified | Enabled admin enforcement for `main` branch protection. Required signed commits remain deferred until the active unsigned PR has merged or local commit signing is configured. Secret-scanning non-provider patterns and validity checks remain blocked by GitHub readback staying `disabled` after PATCH.                                                                    |
...
- Enable required signed commits after PR #80 merges or after local commit signing is configured for future feature branches.
