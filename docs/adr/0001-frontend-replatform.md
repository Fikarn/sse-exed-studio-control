...
Post-acceptance update, 2026-04-25: `v2.2.0` shipped the `Tauri 2 + React 19.2 + TypeScript + Vite` shell as the selected release runtime through the `native:*` release lane, and `v2.2.1` is the current published operator-rollout build after the durable default app-data path fix. The Qt/QML shell remains available only as the fallback runtime during the bounded post-release fallback window; Checkpoint D / Qt retirement is a separate follow-up and is not authorized by this ADR.
...
- after `v2.2.0`, the Tauri `native:*` release lane is authoritative; `v2.2.1` is the current published operator-rollout build, and Qt remains a bounded fallback runtime until Checkpoint D
