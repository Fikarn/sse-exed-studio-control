...
- **Today.** Backspace or Delete on a focused control resets it to its default. That works on:
  ...
  With the mouse, Alt+double-click resets knobs and Lighting's fixture sliders, because a plain double-click opens typed entry. A plain double-click resets Rotation and Beam angle (`ScrubSlider.tsx:207-228, :296-302`; `AudioKnob.tsx:164-169`; `AudioStripGainKey.tsx:68-72`). (Corrected during Slice 3's review: this line also named the bulk sliders, which pass no reset value, so a double-click does not reset them.)
...
| Error "Could not clear Highlight and Solo. Press Esc to try again."                                                  | "… Open Lighting and press the lit Highlight or Solo key." (the review of Slice 3: the message shows only after Lighting is left) | `useLightingFixtureEditor.ts:682`    |
