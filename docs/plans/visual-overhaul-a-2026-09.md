...
Status: **landed 2026-09-07** (commit pending the operator's word). Depends on D2, D5, D6, D7, S1. The design system now owns the A canon: `StateDisplay` (tones ok · attention · error · info, the word, the engine's sentence, the code small, the meta line, the way-out keys, the armed row; 180 px from the chrome token; `data-region="state-display"`, `data-well`), the `Key` family (`Key` with modes command · primary · danger · toggle · momentary · arm · hazard · segmented, `engaged` / `live` lit fills marked `data-lit`, `locked` as a dashed outline at 55 % with `aria-disabled`, the reason as `title` and `aria-description`, `cap` mono uppercase; `ArmKey` with the tag, the amber keyline and the countdown bar keeping `data-testid="audio-arm-countdown"`; `Segmented`; the `useArm` hook: 350 ms dwell, 4.5 s window, Escape, a second press on the same key only), `LampWord` and `Latch`, the well family (`Well`, `Readout` with `doubt` and `empty`, `Field`, `Screen` with the blue preview keyline, `Slider` with the unity notch and the CCT track, `Groove` on the 44 px target column with the 18 px slot, `Meter` mono / stereo with the ramp, the emissive layer, the −18 reference, the peak bloom, the clip lamp, `empty` and `stale`), the plate sections (`PlateHead`, `Section`, `Fields`, `Readouts`, `ControlRow`, `Danger`), `Drawer` at level +3, and `LoadingState` (a still skeleton). `Dialog`, `ConfirmDialog`, `CommandPalette` and `Toast` are re-skinned onto the level +3 recipe with the one focus ring and sentence-case verbs; `Button` aliases `Key mode="command"` (no uppercase, no hover travel); `StatusBadge` speaks the shared vocabulary with the old tone names as aliases; `EmptyState` / `DegradedState` stack to one column under 320 px. Three elevation tokens were added for the recipes that needed them (`keyPrimary`, `keyArmed`, `bar`), and the A type scale gained `font.base.*` so `OperatorLayoutProvider` re-derives every step under the operator UI scale (the existing "operator UI scale reaches portaled overlays" test keeps its meaning: the dialog title now measures 24 × 1.25 = 30 px, old 22 × 1.25 = 27.5). The well scope in `themes.css` scopes only the role inks: text on a well reads the display tokens explicitly, so a key on a well keeps the chassis inks (the Bone state display's keys were white on white before this). Storybook: `Design System/A primitives` — Keys, State displays, Wells, Plate and overlays, and the whole sheet, each in Studio, Graphite and Bone (15 stories), with a theme decorator in `.storybook/preview.ts`; `ui-contract.spec.ts` gains a Storybook page gate that asserts, outright, the 12 px floor, the two families, the four radii, targets ≥ 24 / 28, no negative shadow offsets, no unlit blur over 8 px, no off-policy gradient and 0 contrast failures on every A primitive page (15 of 15 pass). Tests added: `APrimitives.test.tsx` (31: StateDisplay, Key, ArmKey, useArm, LampWord / Latch, wells, Slider / Groove, Meter, plate sections, Drawer). Tests changed, old → new → reason: `StatusBadge.test.tsx` (the seven legacy tones → the five shared tones plus the legacy names as aliases; system §8); `Button.test.tsx` (+ sentence case and no hover travel, asserted on the stylesheet; system §7, §9); `shell.spec.ts` "operator UI scale reaches portaled overlays" (27.5 px → 30 px; the dialog title on the A scale); `themes.contrast.test.ts` (the well's text inks are `--display-text` / `--display-text2`, not the scoped chassis tokens). Baselines: 52 moved (every surface that renders a Button, a badge, an empty state or a toast — Setup, the recovery screens, Lighting, Planning, the DS stories; the Console is untouched) plus 15 new; every moved capture inspected on five contact sheets before the refresh. Numbers moved (ratchets re-seeded): radii outside {4, 8, 12, pill} 7 512 → 7 167 (down on 36 boards), contrast fails 1 313 → 1 270 (down on 26), gradients off policy 3 420 → 3 345, off-family text 324 → 303, type floor up on 9 boards, distinct sizes down on 3; nothing rose. Literal allowlist: `Button.module.css` (colour 6, font-size 1, shadow 1), `CommandPalette.module.css` (font-size 5, shadow 2) and `Toast.module.css` (colour 1) → 0. Not done: the Console, Lighting, Planning and Setup do not compose these yet (S4–S7); lamps, keylines and key edges at 3:1 are measured by the Storybook page gate on the primitives now and by the workspace ratchets from S4.
