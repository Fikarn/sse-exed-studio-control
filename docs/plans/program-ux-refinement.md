...
|     6 | Unified feedback + amber tone + copy cleanup           | C Chrome      | ◐ 6a #134 · 6b/6c todo        | #134           |
...
**Rescope:** 2026-06-08 — Slice 6 sub-sliced 6a/6b/6c (slice heading + scope unchanged; same work, split across PRs for reviewability — the same pattern S3 used for 3a/3b/3c, not a premise change). Slice 6 maps to ~20 findings touching ~15 files and ~25+ baselines across 4 fixtures plus token + layout work, which is too large for one reviewable PR. Split: **6a** = the amber `attention` tone (DS Toast + toastContext + Lighting call-site migration; baseline-neutral) → PR #134; **6b** = operator-grade copy + de-leaked enums (COPY-04, SET-01, SET-05, COPY-09, STA-08/09/10; moves protocol-mismatch/setup-degraded/setup-ready baselines); **6c** = the structural items (CHROME-08 toast offset, STA-05 startup step rail, GLO-05 token-literal extraction with a theme-safe `--color-scrim`). No finding is dropped; SET-04 is a defensive no-op on shipped fixtures (folded into 6b). The naming followed the audit/plan's `attention` (not the agent-suggested `warn`) to match the codebase's `attention`/`attn` family.
