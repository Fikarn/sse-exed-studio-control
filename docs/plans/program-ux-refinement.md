...
|     1 | Token foundation (define)                              | A Foundation  | ◐ in progress | —    |
...
- **Key files:** `frontend/packages/tokens/src/source/tokens.json`, `…/tokens/core.json`, generated `…/generated/tokens.css` (via build); `frontend/app/src/app/OperatorLayoutProvider.module.css:29-31` (the `--operator-font-size-title-{sm,md,lg}` defs — add `title-xl` here); DS primitive CSS (consumers).
...
- **Status:** ◐ in progress. Premise re-confirmed on merged `main` (all namespaces 0 defs; `--size-control-height` exists). Aliases added to `core.json` as **`{references}` + `outputReferences: true`** so they emit `var(--base)` (theme-ready for S3). ok-severity (`--color-accent-green`) → brand `primary-500` (operator decision). title-xl added to the operator scale. **Two scope refinements vs the plan (see Rescope log):** (1) the camelCase refs `--size-controlHeight` (6×) + `--color-surface-borderStrong` (5×) can't be emitted by style-dictionary, so they were fixed in-consumer — S1 is not purely additive token work; (2) DES-03's orphan-pipeline deletion + `tokenValues` repopulation deferred to a follow-up.
