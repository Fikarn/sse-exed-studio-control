...
Operator-reported during the pass: launching the packaged app opened a terminal full of `10054` errors. Two pre-existing defects, both fixed and redeployed: the shell exe was a console-subsystem binary (every launch popped a terminal; the engine child inherited it and spilled stderr onto the operator monitor) — release builds now declare `windows_subsystem = "windows"` and the engine is spawned with `CREATE_NO_WINDOW`; and the classic-slot metering receive loop printed Windows' localhost-UDP `ConnectionReset` (a keepalive to a TotalMix remote that is not listening — remotes 2/3 are currently disabled in TotalMix) once per second per dead slot — that kind is now swallowed like the Global path already did. Relaunch verified: GUI subsystem in the package, app + bridge live, zero console windows. Whether to re-enable TotalMix classic remotes 2/3 (playback/output metering fallback completeness) is left to the operator; audio is authoritative over Global OSC either way.
...
### S6 — Visual language v2 (operator-requested follow-on)

Status: complete (2026-09-01), deployed and live-verified. Replaces the uniform white-on-black text with the app's Console vocabulary on the deck, per the approved proposal in the design artifact (tiers A+B; tier C custom module deferred). Engine: new machine-readable LCD keys `audio_state_{target,bank,mode,dim,talk,solo,gated}`, `audio_strip_N_state` (offline/muted/selected/normal/empty priority), and `audio_strip_N_level` (0-12 fader/gain/volume bucket, `m`-prefixed when muted, `off`/`empty` otherwise); strip text v2 (two lines, uppercase name, dB hero, no target arrow); keys 1-3 static labels (active state reads through color). Export: Companion internal `variable_value` feedbacks per cell (amber active target, green live TALK, warn-yellow SOLO count via inverted eq-0, ember muted / amber selected strips, gated grey-out, bank tint), per-bucket PNG-swap feedbacks drawing the fader-position bar with the unity notch (26 bar variants + off/empty), static icon `png64` backgrounds on all keys, topbars off, fixed type sizes, and 34 shipped custom variables (25 polled audio/workspace + 9 legacy planning/lighting — fixing the "undefined" dial cells on pages 1-3). Assets: 35 PNGs (+.b64 siblings for `include_str!`) rendered by the checked-in `scripts/deck-assets.py` (Pillow, standalone authoring tool). Empirics that shaped it, captured live on this Companion install: `png64` is raw base64 both in button styles and feedback style overrides; internal feedbacks can override color/bgcolor/text/size/png. Validation: fmt/clippy clean, 238 engine tests green (+3), deployed via repackage + Full Reset & Import, then verified on production: PH-1 key rendered solid amber for the live target, a Companion press-API flip to MAIN moved the amber and retargeted all four strip bars/values within the refresh piggyback, icons/bars/full-bleed cells all rendering in the Companion grid (which is the deck's exact render source).
