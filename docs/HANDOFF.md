...
- **No open issues.**
- **Two open Dependabot pull requests.** [#193](https://github.com/Fikarn/sse-exed-studio-control/pull/193) `fuzzysort` 3.1.0 → 4.0.2 is left open by the operator's decision. [#205](https://github.com/Fikarn/sse-exed-studio-control/pull/205) `@babel/core` 7.29.0 → 7.29.7 opened during the sequence and is not decided yet.
- **The queue of 2026-09-01 to 09-11 is worked off.**
  ...
  - #203 (it replaced #197), #196, #195, #202 (it replaced #194) and #192 were merged one at a time on ten green checks.
- **Four open Dependabot alerts**, all development scope: `vitest` and `@vitest/mocker` (medium), `@babel/core` and `esbuild` (low). None reaches the shipped engine or shell. The `supply-chain` job holds the lockfiles to the bars in `scripts/check-npm-audit.mjs`.
