...
- A one-way legacy-import path (`native/rust-engine/src/legacy_import.rs`) remains so that operators migrating from a pre-`v2.0.0` Electron installation can bring their old `db.json` forward on first native launch. This is the only legacy code that is intentionally retained. Since the new pages program's Slice 2 it carries only whether setup is complete and the page to open, and seven lanes and scripts still seed a test engine through it (`SSE_LEGACY_DB_PATH`); Slice 2b retires it once they are seeded through the app's own requests.
