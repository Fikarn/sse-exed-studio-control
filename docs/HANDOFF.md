...
4. Preserve the current frontend replatform checkpoint.
   `Setup/Support` is the verified pilot. The `Lighting` pass has moved to **Direction D — Scene desk** (`docs/redesign/lighting.md` + prototype `docs/redesign/assets/lighting/Lighting-D-Scene-Desk.html` + 4-PR plan `docs/redesign/lighting-direction-d-implementation-plan.md`); D supersedes the closed `Cr — Spatial desk` pass and removes the cue model in favour of scene recall, with `pan/tilt` still intentionally out of scope and the patch inspector still aligned to `dmxStartAddress`, `rigZ`, `beamAngleDegrees`, and `Identify`. PR 1 (Foundation: tokens + fonts + 7 design-system primitives + 92 px shell-header redesign across all workspaces) is landed on `feat/lighting-d-foundation`; PR 3 (workspace rewrite) and PR 4 (cue cleanup) are queued. The `Planning` pass is closed against the checked-in run-of-show timeline / board plan. The `Audio` pass is closed against the locked `Ar+ - Control-room confidence desk` spec in `docs/redesign/audio.md`, including the warning-band trust model, full-width meter bridge, banked strip desk, control-room inspector split, keyboard desk model, degraded-state matrix, and `1920x1080` fallback fit. The broader live Tauri workspace qualification lane now exists as `npm run tauri:workspaces:qualify`; it covers the commissioned dashboard plus live Lighting, Audio, and Planning mutations across restart persistence. The cutover acceptance gate is tracked in `docs/archive/FRONTEND_CUTOVER_PLAN.md`; Checkpoint C is satisfied for published tag `v2.2.0` (`eb166092ad5483a00b6b59137062c86c3193ca53`) with macOS and Windows post-switch target-host release evidence, the final operator-workstation rollout passed on published tag `v2.2.1` (`951a2c4e1f236200f0f017121158bc9969427051`), and Checkpoint D completed on commit `d0205baf52ce02d7d4d24699facd202f3bbba217` with macOS `native:release:mac:local` plus Windows evidence bundle `2026-04-25T07-32-31-463Z`. Validation lane split, runtime selector lockdown, packaging/signing cleanup, Qt source/test removal, parity asset retirement, and the final retirement gate are complete.
