...
- Native and replacement-shell release verification are target-host gates. The `dev-checks` workflow in [.github/workflows/dev-checks.yml](../.github/workflows/dev-checks.yml) runs ten jobs once per pushed commit, on every branch, a pull request reading the checks on its head commit (`format-protocol`, `lint`, `frontend-typecheck`, `frontend-test`, `supply-chain` — since production readiness Slice 12: the npm audit gate and `cargo deny check` — `frontend-e2e`, `rust`, `rust-coverage` — since Slice 13 — `tauri-foundation`, `qualification`) and is required merge hygiene on `main`; target-host evidence remains the release acceptance mechanism. The frontend-e2e job runs every behaviour case and the UI contract on its Linux runner but compares no screenshots and samples no contrast there (since the new pages program's Slice SW): the committed captures are win32 only, at `2560×1440`, and they and the contrast are checked on the studio workstation before each push. The `rust` job extends to `native:acceptance`, whose harness runs the engine in simulated audio input mode (a runner has no TotalMix, and the harness never writes to a console); `qualification` runs both Tauri qualifications under xvfb with extended timeouts and the audio probe skipped for the same reason.
- **The studio workstation runs a build of `main` since 2026-09-23 14:15, not `v2.2.1`** (the program's builds since 2026-09-21, the operator's decision that day: "Move to the new build and use real data"). `release\native\windows\SSE ExEd Studio Control Native\` holds the packaged build of `main` at `98bbb06`: the program (#201), the fixes after it (#204, among them `SYNC NEEDED`) and the Dependabot merges of that day, among them Tauri 2.11.6. It was promoted as `6fbae80` on 2026-09-21 at 12:56, replaced by `db2df4d` at 13:37, by `7b85f04`, the bridge fix, on 2026-09-22 at 11:57, by `e62bcba` on 2026-09-23 at 08:15, and by `98bbb06` at 14:15 (the `e62bcba` build is kept at `release\native\windows.e62bcba-2026-09-23\` as the rollback, the `7b85f04` build at `release\native\windows.7b85f04-2026-09-23\`, the `db2df4d` build at `release\native\windows.db2df4d-2026-09-21\`, the `6fbae80` build at `release\native\windows.6fbae80-2026-09-21\`, the S2-era build at `release\native\windows.s2-era-2026-09-10\`; sizes and sha256 in the production readiness ledger, After the program), running on the real app-data `%APPDATA%\ExEd Studio Control Native`, whose database is at schema 7 since 2026-09-18 08:27 — older builds refuse it. It opens fullscreen on display 3 (`\\.\DISPLAY3`, 2560×1440 at 100 % scaling). `scripts/native-package.mjs` rebuilds `release\native\windows` from scratch, so on this machine it runs only with that folder moved aside first (the procedure is in the production readiness ledger), and no engine from the repository may be started against the default app-data directory without `SSE_APP_DATA_DIR` pointing elsewhere.
...
- **The new pages program ([docs/plans/new-pages-2026-09.md](plans/new-pages-2026-09.md), from 2026-09-24, the operator's request of that day).** Planning is removed, then every keyboard shortcut, then two workspaces are added, Cameras and Teleprompter — all before the studio walk, which then runs on the program's last build with a rewritten checklist. The ledger's per-slice Status lines are authoritative. Slice 1 (Planning leaves the screen) landed by #211 (squash `e8d43c5`, 2026-09-25): the header has three tabs, Setup / Support · Lighting · Audio. Slice 2 (Planning leaves the hardware link) landed by #213 (squash `23a2f9a`, 2026-09-25): the 18 `planning.*` methods, `commissioning.seedPlanningDemo` and `planning.changed` leave the contract; the saved data goes to schema 8, which drops Planning's four tables and every `planning.*` setting and opens a saved Planning page on the Console, behind the verified `pre-migration` copy; backup archives are format 5 with no Planning, and older archives and database backups restore with their Planning part skipped (Verify says so); the Stream Deck profile has two pages, LIGHTS (1) and AUDIO (2), which follow the app, and no PROJECTS or TASKS; the legacy `db.json` import carried only the setup flag and the page to open. Slice 2b (the `db.json` import retired, branch `new-pages/s2b-db-json-import-retired`) removes it: `storage.importLegacyDb` leaves protocol 2, a `db.json` offered to Verify or Restore is refused by name, a left-over one is named in one warning line at a start and nothing is imported, and every lane seeds its saved data through the app's own requests and starts its engines and shells hardened (a bridge port of their own, `SSE_SAFE_START=1`, the simulated console); it landed by #215 (squash `f3b588c`, 2026-09-26). Slice 3 (no keyboard shortcuts, branch `new-pages/s3-no-shortcuts`, landed by #216, squash `da18125`, 2026-09-26) removes every key Studio Control bound and every key hint — the workspace keys, the command palette, the shortcut guide, the Console's, Lighting's and Setup's page keys, talkback's `T`, F2 and the keys held while pointing — and puts a control on the screen where a key was the only way: the window keys in Setup / Support › Workstation and on the recovery screens, the Console's bank keys, Lighting's Undo key, Stop on Find and Add to selection, and Reset in typed entry; the native shell switches WebView2's reload, find, print and zoom keys off, and `scripts/check-no-shortcuts.mjs` keeps key bindings and key hints out. Slice SW (branch `new-pages/sw-windows-2560-only`, the operator's ruling D22 of 2026-09-26: Studio Control always runs at `2560×1440`, fullscreen on an external display, on Windows) removes everything that existed only for another size or system: the smaller layouts (compact chrome and density, the Console's compact banks, Lighting's narrow drawer), Studio Preview, the Windowed key and the shell's windowed layout (the window always opens fullscreen), the size-only Playwright cases and fixture, the linux and darwin captures and the win32 ones at other sizes, CI's screenshot comparison and contrast sampling, and the macOS packaging, signing, release assets and copy. None of this reaches the workstation before the program's close-out: the live app stays on `98bbb06` at schema 7 until then, and the close-out's build upgrades the saved data and comes with a new Stream Deck profile to import with Full Reset & Import.
- Responsive operator layout support landed in [PR #71](https://github.com/Fikarn/sse-exed-studio-control/pull/71) on `2026-05-03` (`4af7e8b8427cff78837054326478e1a67398154c`). Lighting now has logical CSS-pixel layout modes, mode-keyed column persistence, toolbar priority overflow, a narrow inspector drawer, separate stage zoom controls, shell-owned window layout persistence, and Scaled Studio Preview for current-hardware human review. (The layout modes below the studio layout, the narrow drawer, Studio Preview and the windowed layout went in the new pages program's Slice SW; the column widths and the stage zoom are still saved under the studio layout's names.)
...
- Audio Phase 3 gold-standard polish landed in 8 slices (`#88`–`#95`, 2026-05-23) plus a 35-item follow-up audit pass merged via [PR #97](https://github.com/Fikarn/sse-exed-studio-control/pull/97) on `2026-05-24`. The audio page now reads one warn yellow / one engaged amber / one talkback green, mini-graph and full-graph processing surfaces share a hardware-readout vocabulary, the Playback strips no longer read as hollow, Outputs Mute lives next to "Bus level" inside the bus panel, peak-hold readouts go neutral grey in the safe zone, and one full-width banner at most renders at the top of the workspace at any time. Plan checked in at [docs/plans/audio-phase-3-gold-standard-polish.md](plans/audio-phase-3-gold-standard-polish.md); follow-up ledger at [docs/plans/audio-ui-phase-3-followup-fixes.md](plans/audio-ui-phase-3-followup-fixes.md); honest scoring + closeout deltas in [docs/plans/audio-ui-gold-standard-progress.md](plans/audio-ui-gold-standard-progress.md).
- The 2026-06-06 → 2026-06-10 program-wide UI/UX refinement plan ([docs/archive/program-ux-refinement.md](archive/program-ux-refinement.md)) completed across PRs [#125](https://github.com/Fikarn/sse-exed-studio-control/pull/125)–[#161](https://github.com/Fikarn/sse-exed-studio-control/pull/161), followed by the round-2 runtime-audit fix cycle and close-out (PRs [#162](https://github.com/Fikarn/sse-exed-studio-control/pull/162)–[#168](https://github.com/Fikarn/sse-exed-studio-control/pull/168)). Every operator surface now follows the app-wide Studio/Graphite/Bone `[data-theme]` system with per-theme + designed-state visual baselines, the shared chrome/control/feedback contracts, and a literal-free token discipline; both audits (`docs/archive/program-ux-audit-2026-06-05.md`, 77 findings; `docs/archive/program-ux-audit-round-2-2026-06-10.md`, 10 findings) are fully discharged — every finding fixed, refuted with recorded evidence, or closed in the audits' own status tables. The plan doc's per-slice Status lines are the authoritative per-slice record (premise corrections, operator decisions, baseline movements).
- No pre-`v2.0.0` code is retained. The last piece, the one-way `db.json` import (`native/rust-engine/src/legacy_import.rs`) for operators migrating from an Electron installation, is retired by the new pages program's Slice 2b, with `storage.importLegacyDb`, the start-up auto-import, `SSE_LEGACY_DB_PATH` and `SSE_DISABLE_AUTO_IMPORT`; the seven lanes and scripts that seeded a test engine through it now use `settings.update` and `commissioning.update` with `overrideProbes`, and the development parity fixtures write their settings directly.
...
5. `docs/HARDWARE_PROFILE.md` — the room this is built for. `2560×1440`, fullscreen
   on a fixed second monitor, on Windows, is the only size and system (operator
   rulings: plan D4, absolute since 2026-09-26).
...
- `docs/plans/production-readiness-2026-09.md` — the execution record of the
  ...
  the 32 findings of the 2026-09-10 readiness audit, all landed by 2026-09-21; on
  `main` since 2026-09-23 by #201, its commits under the tag `archive/program-2026-09`).
  Its traceability table names each finding's commit and the guard that holds it;
  Appendix A says which lane runs each guard; Appendix B is the operator hardware
  checklist: items 1, 6 and 7 are signed, items 2–5 wait for the studio walk, and
  item 8 follows them.
...
1. Keep target-host evidence complete.
   Windows 11 `x64` is the only release host (macOS left with the new pages program's Slice SW). Do not claim a cutover gate from stale local artifacts or from a prepared-only installer layout.
...
4. Preserve the current frontend replatform checkpoint.
   `Setup/Support` is the verified pilot. `Lighting` Direction D and the premium Waves 24-34 pass are complete: D supersedes the closed `Cr — Spatial desk` pass, removes the cue model in favour of scene recall, keeps `pan/tilt` intentionally out of scope, and now includes preview editing, manual fade recall, selection chips, compact DMX, Highlight/Solo/Find, color/reorder polish, and per-attribute palette pools. All 28 premium findings in [docs/redesign/lighting-d-premium-plan.md](./redesign/lighting-d-premium-plan.md) are closed; Wave 34 PR #65 landed with macOS and Windows target-host validation, and future Lighting work should start from new issues instead of reopening the wave plan. The `Planning` pass was closed against the checked-in run-of-show timeline / board plan; Planning itself left the app in the new pages program (2026-09, Slices 1 and 2). The `Audio` pass is closed against the locked `Ar+ - Control-room confidence desk` spec in `docs/redesign/audio.md`, including the warning-band trust model, full-width meter bridge, banked strip desk, control-room inspector split, keyboard desk model, degraded-state matrix, and `1920x1080` fallback fit. The broader live Tauri workspace qualification lane now exists as `npm run tauri:workspaces:qualify`; it covers the commissioned dashboard plus live Lighting and Audio mutations across restart persistence (and Planning's until the new pages program's Slice 1). The cutover acceptance gate is tracked in `docs/archive/FRONTEND_CUTOVER_PLAN.md`; Checkpoint C is satisfied for published tag `v2.2.0` (`eb166092ad5483a00b6b59137062c86c3193ca53`), the final operator-workstation rollout passed on published tag `v2.2.1` (`951a2c4e1f236200f0f017121158bc9969427051`), and Checkpoint D completed on commit `d0205baf52ce02d7d4d24699facd202f3bbba217`. Validation lane split, runtime selector lockdown, packaging/signing cleanup, Qt source/test removal, parity asset retirement, and the final retirement gate are complete.
...
6. Keep one layout.
   The studio surface is a logical `2560x1440` operator composition, fullscreen on the fixed studio monitor, on Windows, and it is the only layout: the new pages program's Slice SW removed the smaller layouts, the compact chrome and density, Lighting's narrow drawer, Scaled Studio Preview and the windowed layout (the operator's ruling D22 of 2026-09-26). Do not write a rule for another size. The UI scale (90–125 %) is the operator's preference, not a layout. The fixed studio monitor is the authority for physical-size readability and ergonomics.
...
9. **The 2026-09 program landed on `main` on 2026-09-23**, by one squash pull request: [#201](https://github.com/Fikarn/sse-exed-studio-control/pull/201), squash commit `1911559`. The 127 commits, which the ledgers cite by hash and by subject, are kept unchanged under the tag `archive/program-2026-09`, and `git diff archive/program-2026-09 1911559` is empty. All ten CI jobs are required checks on `main` from now on. Work continues on branches cut from `main` and lands by pull request. The record is in the production readiness ledger, After the program. The fixes after it landed the same day by [#204](https://github.com/Fikarn/sse-exed-studio-control/pull/204) (squash `1104f0e`, its commits under the tag `archive/after-the-program-2026-09`), followed by the Dependabot queue, one pull request at a time (Execution Queue). The three lower stack branches below were deleted from GitHub the same day; every commit of theirs is inside the tag `archive/program-2026-09`. What follows is how it stood before the landing.

   Four bodies of work sat stacked on `origin/main`, each containing the one below it, all pushed to GitHub and none merged. Counts as of `2026-09-23`, before the landing:
   ...
   | `production-readiness-2026-09`  | 127                     | the above + the 16-slice production readiness remediation (S0–S15) and eleven fixes that are not slices |
...
As of `2026-09-23`, after the landings (the record is in the production readiness ledger, After the program):

- **No open issues.**
- **One open Dependabot pull request.** [#193](https://github.com/Fikarn/sse-exed-studio-control/pull/193) `fuzzysort` 3.1.0 → 4.0.2 is left open by the operator's decision.
- **The queue of 2026-09-01 to 09-11 is worked off.**
  - #198 and #199 were closed as overtaken.
  - #200 (`vitest`) was closed, because Dependabot cannot rebase a pull request whose config entry the program replaced. A fresh one comes under the current `.github/dependabot.yml`, where every entry has `rebase-strategy: disabled` (a rebase is asked for with `@dependabot rebase`).
  - #203 (it replaced #197), #196, #195, #202 (it replaced #194) and #192 were merged one at a time on ten green checks, and then #205 (`@babel/core` 7.29.7, `f583f41`), which Dependabot opened during the sequence.
- **Three open Dependabot alerts**, all development scope: `vitest` and `@vitest/mocker` (medium), `esbuild` (low). None reaches the shipped engine or shell. The `supply-chain` job holds the lockfiles to the bars in `scripts/check-npm-audit.mjs`.
...
- **Baselines are win32 only, at 2560×1440** (59: 19 visual-review, 40 Storybook), compared on the studio workstation before each push; CI compares none. The new pages program's Slice SW removed the `linux` and `darwin` captures and the win32 ones at other sizes; `docs/DEVELOPMENT.md §2b` has the refresh steps.
...
- (Historical — the flags below predate the #99–#110 remediation.) `tauri:visual:review` is now a thin Playwright wrapper: it accepts no `--port`/`--fixtures`/`--sizes` flags, port `4173` is hardwired via `frontend/app/playwright.config.ts` (`--strictPort`; free the port before running), and its summary records coverage/baselines rather than fidelity metrics. Scaled Studio Preview capture via `operatorReview=studio` still applies. If the live shell falls into `ENGINE_READY_TIMEOUT` after broad frontend work, use the shell's own retry flow before judging the surface.
...
- Built-in-display review opens with `?operatorReview=studio` in the page's address. (The command-palette entries `Studio Preview: Enter 2560x1440 Review` and `Studio Preview: Exit Review` are gone with the palette since the new pages program's Slice 3, 2026-09, and the app no longer remembers the preview.)
...
- The implementation plan is preserved at [docs/archive/lighting-fixture-catalog-implementation.md](./archive/lighting-fixture-catalog-implementation.md), with the completed scope documented at the top of that file.
...
- The implementation plan is preserved at [docs/archive/stage-plot-fixture-identity-implementation.md](./archive/stage-plot-fixture-identity-implementation.md).
...
- Obsolete branch, no longer on origin (checked 2026-09-23; kept under the tag `archive/audio-meter-phase-2-handoff-2026-05-20`, archived 2026-05-24): `claude/audio-meter-phase-2` was a 2026-05-20 checkpoint of an earlier Phase 1b audio meter architecture (`audio_backend.rs` refactor, protocol changes, `LiveAudioStereoMeter` + `meterStore` split, ballistics module). It sits 21,988 lines BEHIND main and removes `rme_totalmix_osc.rs` which the shipping engine depends on — Phase 2 took the meter architecture in a different direction. Safe to delete or tag-and-archive; not landable on current main without redoing the entire Phase 2 close-out on top.
...
node scripts/ui-census.mjs                    # the 63-board UI contract (66 before Slice SW, 81 before Slice 1); ~4 min
...
GitHub Actions runs the ten-job workflow in [.github/workflows/dev-checks.yml](../.github/workflows/dev-checks.yml) once per pushed commit, on every branch, Dependabot's included (since 2026-09-25; a pull request reads the checks on its head commit, and one from a fork gets no run): `format-protocol`, `lint`, `frontend-typecheck`, `frontend-test`, `frontend-e2e`, `rust` (rustfmt + clippy + cargo test + the dev-fixtures tests + `native:acceptance`), `tauri-foundation`, `qualification`, — since production readiness Slice 12 — `supply-chain` and — since Slice 13 — `rust-coverage` (see "Current Operating Truth" above for the per-job detail). Since Slice 13 `frontend-e2e` fails on Playwright's `default` project alone; the wall-clock cases named in `frontend/app/tests/quarantine.json` run in a step that reports and never fails the job, and since Slice 15 there are none. All ten are required checks on `main` since 2026-09-23, with "require branches to be up to date" on (production readiness Appendix B item 6). Since the new pages program's Slice SW `frontend-e2e` compares no captures and samples no contrast on its Linux runner; the workstation's Playwright run before each push does both. A `v*` tag, or a manual run, additionally runs `release-evidence.yml` (bundle, SHA256 manifest and SBOMs from a clean Windows runner; it publishes nothing). Target-host release evidence on Windows 11 `x64` remains the release acceptance mechanism for this repo; CI failures are merge blockers, not release evidence.
