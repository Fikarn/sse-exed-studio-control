...
- `v2.2.1` is the current published Tauri shipping runtime through the `native:*` release lane. `v2.2.0` completed Checkpoint C, then `v2.2.1` patched operator app-data defaults for workstation rollout. The bounded fallback window is closed in GitHub issue #3, and Checkpoint D planning is tracked in GitHub issue #5. `scripts/native-release-runtime.json` selects `tauri` for the shipping release path; `native/qt-shell/` remains present until a Checkpoint D change removes or archives it.
...
- the selected shell stack is `Tauri 2 + React 19.2 + TypeScript + Vite`; Qt fallback retirement must go through Checkpoint D issue #5
