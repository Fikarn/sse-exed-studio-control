...
- **Baselines are per-platform.** `win32` and `linux` are current (`linux` since production readiness Slice 13), except 12 win32 captures at sizes other than 2560×1440 that the fixture double's health words moved on 2026-09-22, which keep their old baselines by the operator's decision (the 15 win32 ones at 2560×1440 were refreshed then, and the 27 linux ones in `8e7d8a5` on 2026-09-23); since the new pages program's Slice 1 they are 13 (six went with the Planning board, and that slice moved seven more; its ledger names them); `darwin` is not, and by the operator's ruling of 2026-09-18 nobody works on it. See Current Blockers item 9.
...
cd frontend/app && npx playwright test        # the blocking default project: 391 cases since the new pages program's Slice 1 (441 before); 82 win32 captures
node scripts/ui-census.mjs                    # the 66-board UI contract (81 before that slice); ~4 min
...
GitHub Actions runs the ten-job workflow in [.github/workflows/dev-checks.yml](../.github/workflows/dev-checks.yml) once per pushed commit, on every branch, Dependabot's included (since 2026-09-25; a pull request reads the checks on its head commit, and one from a fork gets no run): `format-protocol`, `lint`, `frontend-typecheck`, `frontend-test`, `frontend-e2e`, `rust` (rustfmt + clippy + cargo test + the dev-fixtures tests + `native:acceptance`), `tauri-foundation`, `qualification`, — since production readiness Slice 12 — `supply-chain` and — since Slice 13 — `rust-coverage` (see "Current Operating Truth" above for the per-job detail). Since Slice 13 `frontend-e2e` fails on Playwright's `default` project alone; the wall-clock cases named in `frontend/app/tests/quarantine.json` run in a step that reports and never fails the job, and since Slice 15 there are none. All ten are required checks on `main` since 2026-09-23, with "require branches to be up to date" on (production readiness Appendix B item 6). A `v*` tag, or a manual run, additionally runs `release-evidence.yml` (bundle, SHA256 manifest and SBOMs from clean Windows and macOS runners; it publishes nothing). Target-host release evidence on macOS Apple Silicon and Windows 11 `x64` remains the release acceptance mechanism for this repo; CI failures are merge blockers, not release evidence.
