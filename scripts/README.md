...
| `native-runtime-harness.mjs`                                                                                                                                                                                                                                     | the lanes' hardening (`hardenedLaneEnv`, `laneEnvRefusal`, `laneProcessEnv`, and the engine start that refuses an environment without it) in `native-lanes.test.mjs` (new pages program, Slice 2b; the scratch app-data check and the refusal of a lane's own `liveConsole` since its review, 2026-09-25); the rest is _accepted gap_ (exercised by acceptance lanes)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| every script that starts the app for a lane: the hardware-link lanes, `native-package.mjs`, `legacy/tauri-package-candidate.mjs`, `tauri-setup-support-qualification.mjs`, `tauri-workspace-qualification.mjs`, `tauri-smoke.mjs`                                | `native-lanes.test.mjs` (Slice 2b, when the db.json import was retired, and its review, 2026-09-25): no lane names the import's variables or fixtures, and every publish carries the probe override a fresh hardware link needs; every process they start that is not a named tool (`reg.exe`, `powershell`, `taskkill`) — the installer lane's QtIFW runs, which start the installed app's first-launch check, included — gets its environment from `laneProcessEnv`, which refuses one without an absolute scratch app-data and log folder outside the platform's app data (`%APPDATA%\ExEd Studio Control Native` on Windows), a bridge port of the lane's own (never the live app's 38201), `SSE_SAFE_START` and the simulated console; only the Setup/Support lane's step 8 may leave the safe start out, and only `acceptanceEngineEnv` (the live console lane's engines, `SSE_NATIVE_ACCEPTANCE_LIVE_CONSOLE=1`) the simulated console |
| the scripts that delete or rewrite folders under `release/`: `native-package.mjs`, `native-installer.mjs`, `native-update-repo.mjs`, `native-installer-acceptance.mjs`                                                                                           | `native-lanes.test.mjs` (the review of Slice 2b, 2026-09-25), which reads them as text and never imports them: each does nothing when imported — its work sits behind one main-module check that compares real paths — and `native-package.mjs` refuses to remove `release/native/windows` while Studio Control runs from it or anything answers on 127.0.0.1:38201 (on the studio workstation, move the folder aside first: keep-and-restore); the refusal itself was proved only in scratch copies                                                                                                                                                                                                                                                                                                                                                                                                                                          |
