...
Production readiness remediation (program `production-readiness-2026-09`, branch of the same name, started 2026-09-10): entries below tagged "(2026-09 production readiness, Slice N)" belong to it. The execution record is `docs/plans/production-readiness-2026-09.md`; its Appendix B is the operator hardware checklist.

### Added

- **The database is checked at every start and backed up on its own (2026-09 production readiness, Slice 3 — findings F02, F13, F21).** The app opened whatever sat at `studio-control.sqlite3`, ran the schema migrations on it in place and reported the integrity check without acting on it; no copy of the database existed anywhere unless the operator exported a support archive by hand, and SQLite was allowed to acknowledge a commit before it had reached the disk. Now the file is integrity-checked before anything touches it — a file SQLite refuses, or one whose check fails, stops the start-up at the recovery surface as `SAVED DATA NEEDS ATTENTION` (code `STORAGE_CORRUPT`) with a sentence that names the file and the newest database backup, and the file is left exactly as it was. Such an engine-side start-up failure now reaches the recovery display at all: the shell registered its event listener only after asking for the engine to start, so a failure raised within milliseconds of the spawn was dropped and the display showed a ready timeout ten seconds later; the listener is now in place before the engine exists, and a failure the engine reported is never replaced by that timeout. The engine writes verified copies of the whole database to the app-data `backups` directory (`db-<UTC timestamp>-<reason>.sqlite3`, one self-contained file each, checked before it counts and deleted if the check fails): before any schema upgrade (`pre-migration`, keeps 5), five minutes after start and every 24 h (`daily`, keeps 14) and at every graceful close (`shutdown`, keeps 3); `pre-restore` copies (keeps 5) are reserved for the database restore that arrives with Slice 7. A schema migration that rewrites a stored value and finds it unreadable now fails as `STORAGE_MIGRATION_FAILED` with the key named and nothing changed — the pre-migration copy is already on disk — instead of silently skipping the value. Every commit now waits for the disk (`PRAGMA synchronous = FULL`). Until Slice 7 lands the Support surface's restore of database backups, `docs/OPERATIONS.md` ("The app fails before the dashboard") has the manual restore. Covered by the storage tests (a refused junk file and a refused damaged page, a verified backup, retention per reason, the pre-migration copy of a v5 database, a rolled-back blob migration, the synchronous pragma), the bootstrap tests (the code, the sentence and the log line) and the new `corrupt-db` scenario of the setup-support qualification lane.
- **Loading sample planning asks first (2026-09 audit remediation, Slice 12 — operator decision 8).** The Setup › Import step's "Load sample planning" button seeded demo projects, tasks and schedule blocks into Planning on a single click, right next to the profile download. It now opens a confirmation that says what will be added and that it sits next to real planning data.
- **Closing the window asks first, and the engine stops gracefully (2026-09 audit remediation, Slice 11).** The README and OPERATIONS promised a close confirmation that did not exist: the X button killed the shell and the engine child with it, mid-hold talkback included. The native shell now prevents the close and raises `shell://close-requested`; the app answers with "Close Studio Control?" — after the lighting unsaved-scene guard when it applies — in every shell state, startup and recovery included. Confirming calls the new `shell_confirm_close` command, which stops the engine gracefully (its stdin closes, the engine's loop ends and releases any talkback hold, two seconds of grace before a kill), marks the close confirmed and closes the window, persisting the window preferences on the way out. Engine restarts use the same graceful stop. `SSE_SHELL_SKIP_CLOSE_CONFIRM=1` bypasses the dialog for automation. Covered by a close-policy unit test and a graceful-wait test in the shell crate, and a Playwright case that drives the browser hook through dialog, Cancel, Escape and confirm; the native close path itself is operator-verified (checklist B8), not CI-covered.
...
- **The Stream Deck bridge answers only the exported profile (2026-09 production readiness, Slice 2 — findings F01, F06, F28).** The local HTTP bridge Companion drives the deck through accepted any request from any program on the workstation, a browser page included: one unauthenticated `POST` could delete the selected project or switch every light. The app now creates a bridge token once per install (`control-surface.token` in the app-data directory) and refuses every request that does not carry it (`401`), any request from a browser origin (`403`) and any request addressed to another host (`400`); the exported Stream Deck profile carries the token on every key, dial and LCD poll, so **the profile must be exported again from Setup step 1 and imported once with Full Reset & Import** — a profile exported before this change is refused (`docs/OPERATIONS.md`, "Stream Deck Audio Surface"). The bridge also stops taking whatever it is sent: headers over 8 KiB (`431`), bodies over 16 KiB (`413`), bodies without a length or with chunked encoding (`411`) and requests that do not finish within a second (`408`) are refused before anything is read into memory; a fixed pool of four workers with a queue of sixteen answers `503` instead of starting a thread per connection; a handler that panics no longer takes a worker with it; refusals are logged at most once per status per minute. LCD keys in the query string are percent-decoded, so `%2F` reaches the lookup as `/` instead of being matched verbatim. Covered by the bridge unit tests (over real loopback sockets, the saturated pool included), an export test that checks the token sits on every profile request and nowhere else, and the bridge qualification lane (`native:bridge:win:verify`), which now sends the token, proves each refusal and checks the exported profile.
- **The dev parity-fixture method is compiled out of release engines (2026-09 production readiness, Slice 1 — finding F04).** `dev.parityFixture.load`, the method that loads a bundled parity database over the operator's data, shipped in every engine, and its default replaced existing planning data. The handler and the bundled fixture payloads now compile only into engines built with the `dev-fixtures` cargo feature (`npm run native:engine:build:dev-fixtures`); a release engine keeps the method in the protocol contract and answers `METHOD_UNAVAILABLE` without touching anything. Where the method is available it merges by default, and `replaceExistingData: true` is the only way to replace existing planning data. No lane calls the method today.
- **A bare engine launch keeps its data in the platform app-data directory (2026-09 production readiness, Slice 1 — finding F22).** With no `SSE_APP_DATA_DIR` the engine used `./native-runtime`, a path relative to whatever directory it was started from. It now resolves the same durable per-user location the shell uses (`%APPDATA%\ExEd Studio Control Native` on Windows, `~/Library/Application Support/ExEd Studio Control Native` on macOS, `$XDG_DATA_HOME` or `~/.local/share` elsewhere) and refuses to start with a clear message when the platform provides no base. The shell always passes the directory explicitly, so a shell-started engine is unchanged.
- **The engine no longer imports a `db.json` from its working directory (2026-09 production readiness, Slice 1 — finding F23).** When its planning tables were empty the engine auto-imported `<cwd>/data/db.json`, so a packaged engine could seed the operator's database from wherever it was launched. The only auto-import sources are now `SSE_LEGACY_DB_PATH` and a file staged at `<app-data>/import/db.json`; `SSE_DISABLE_AUTO_IMPORT=1` still turns the import off.
- **The frontend dev server listens on loopback only (2026-09 production readiness, Slice 1 — finding F24).** Vite bound `0.0.0.0`, so the dev shell's front end and its test bridge were reachable from the studio LAN during `tauri dev`. It binds `127.0.0.1` now, and a scripts test fails if any local server surface goes back to every interface.
- **CI runs on every branch push (2026-09 production readiness, Slice 0 — finding F03).** `dev-checks.yml` triggered on pull requests and on pushes to `main` only, so no job had ever run on the three stacked 2026-09 branches. It now also runs on every branch push (Dependabot branches excluded — they already run per pull request), so a pushed slice is checked before a pull request exists. The first run on the stack is expected to fail `frontend-e2e` on the stale linux visual baselines; that refresh reads the run's `playwright-test-results` artifact.
- **The Console and the Lighting panel read at the size the rest of the app reads, and nothing you press is too small to hit (visual overhaul A, Slice 11).** Those two screens still carried print down to 9.5 px and 8.5 px where every other screen in the program stops at 12 — the smallest text in the app, on the two screens you read during a take. There is none left below 12 px on either, and each screen now speaks in eight type sizes or fewer instead of twelve, so a panel reads as a ladder rather than a spread. Nothing you can press is under 24 px any more. The lighting panel's level bars were 14 px tall to drag; the bar looks the same and the column you can grab is now a full 24. The labels you drag sideways to nudge a fixture's position got the same. The ✕ that drops a fixture from the selection went from 16 px to 24, and the pin on a scene tile, the plot's three view slots, the rename key beside a fixture's name, the chips in the scene list and the Console's equaliser band keys all came up to the floor with it. Corners on both screens now come from the same four radii as everywhere else — 29 shapes on Lighting and 10 on the Console were drawing their own. Two things surfaced on the way through: the lighting palettes had stopped printing their names — "Low", "Quarter", "Half" and "Full" were squeezed to nothing by the row of keys beside them, so a palette could only be told apart by its number, and the keys sit under the name now; and two places on the Console had been built around the smaller print — the meters printed their dBFS scale in a lane too narrow to hold it at a readable size, and on the 1920 screen the send card's four keys were squeezed until "PRE FADER" lost its last letter. The scale marks have their room, and the four keys fold into two rows when the card is narrow.
- **One name for everything (visual overhaul A, Slice 11 — close-out).** The overhaul left a period where the old names and the new ones both worked, so nothing broke while the surfaces moved across. That period is over. Sixteen type, radius and motion sizes that had two names each now have one, in every stylesheet — the values were identical, so nothing on screen moves — and a fifth corner size that nothing had ever drawn with is gone, so the app rounds a corner at four sizes and calls each of them by one name. The six pre-A state words (healthy, ready, connected, degraded, warning, idle) are gone; the app says ok, attention, error, info and neutral everywhere, and the two startup screens no longer translate between the two on their way to a lamp. Eight Lighting components that nothing has rendered since the cluster replaced them, and four style rules with nothing left to style, were deleted. The operating manual was still sending you to a "Console badge" in a top bar that no longer exists, calling the Sync key by a name it stopped using, and calling the RME desk "the console" — which is the name of the screen you read it on; it now says what the app says. The hardware profile's density table quoted panel widths from two revisions ago and now quotes the ones the app is built and tested against.

- **Every shadow on screen comes from one light; controls no longer jump on hover; nothing animates on an idle surface; reduced-motion settings are respected (visual overhaul A, Slice 9).** The frosted-glass panels are gone. The active-scene pill, the chips on the stage plot, the plot's symbol key, its empty-state card, its view controls and the panel a dialog sits on were all translucent, blurred and dropped a soft 24–40 px shadow; each of them now sits on the same solid plate as everything else in the app, lit from above with the one large shadow the room's light allows, and their small print reads at full strength instead of through frosting. Glows are one size each now: a lamp glows at 10 px wherever it appears, an engaged amber control at 14, a live green one at 18 — the plot pill's dot, the tab underline, the equaliser handle, the fader cap and the recalled scene tile were at five different sizes between them. Nine panels that floated with a 28–88 px shadow now use the one drawer shadow, including the Lighting drawer, which was lit from the right instead of from above. Four things that were not floating at all — a Planning board card, the card you are dragging onto, a card with a timer running and Setup's current step — say so with an edge instead of a shadow. The Lighting scene tile's ACTIVE and MODIFIED dots stopped pulsing: they were the only thing still moving on a Lighting page at rest, and the dot's colour already says which state it is in. Nothing under the pointer moves any more — the lighting sliders' handles used to swell, the backup rows and rail buttons in Setup and on the recovery screen used to lift a pixel, and the colour swatch used to grow; all of them now brighten an edge and stay put. A press still moves, because a key that does not travel does not feel pressed. Asking the operating system for reduced motion stops everything, the loading skeleton included.

- **Operator copy names the desk, the bridge and the deck; every state says what happened and what to do next; key caps read like the Stream Deck's (visual overhaul A, Slice 8).** The app now uses one name for each piece of gear in the room. The desk is the RME UFX III and TotalMix is the software in front of it, so the audio state display says the desk is unreachable rather than "the console", Setup's addresses are TotalMix's rather than "Audio console" and "Audio send host", and the recall report says what the desk adjusted. The bridge is the Apollo lighting bridge and nothing else. The deck is the Stream Deck+ — the header lamp said "Surface", the Verify step asked you to press buttons on "the desk", and the publish record called the deck a "Control surface". The app itself is Studio Control, not "the shell". Words that belong to the program and not to the room are gone from the screen: the About readout said "Engine" and now says Hardware link, the Map step said "Transport: fixture" and now says Hardware link: sample data, the lighting preview talked about a buffer and about committing and now talks about the preview and about saving, clearing Highlight and Solo no longer reports "Failed to clear overlay", and "toast", "adapters", "runtime paths", "persistence", "handshake", "dataset" and "no-op" have left the operator's screen entirely. Every state now says what to do as well as what happened, and the key that does it is in the same place you read the sentence: the stage plot's lock note reads "locked · the bridge is not answering · Open Setup", patch mode tells you P leaves it, trying to save with no scene recalled tells you ⇧S saves the rig as a new one, an empty board column tells you to press All, a saved view tells you which Shift+digit brings it back, an overlapping card tells you to drag it or nudge it 15 minutes with an arrow key, and every action that fails names the key you pressed. Numbers carry their unit with a space before it — 76 %, 3200 K, 50°, 0 s, 48 V, −20 dB, U1 — and a count of one is no longer "1 tasks". Commands read as verb then object in ordinary sentence case: Fit room, Fill screen, Add task, Export backup, Run all probes, Clear the selection, Set value. Two things the app was saying that were not true were fixed on the way through: a failed start printed a fault code the engine had never reported and claimed it "failed at runtime" when no stage was reported, and the command palette printed Ctrl+1 for Lighting where Lighting is Ctrl+2. The sentences the engine itself writes are unchanged and still print verbatim with their code small beneath them.

- **Setup says where commissioning stands, and a step is a page that fits (visual overhaul A, Slice 7).** The utility row is gone. Down the left, where every workspace keeps its own, commissioning now says what it is — **READY**, **DEGRADED** or **SETUP REQUIRED** — with the sentence the app itself reports, which step you are on, how the probes came back and when the last backup was taken, and the one key that gets you out: Run all probes when one is not green, or Start with Import profile before commissioning. Under it: Runner or Support, the five steps in the order they are done with a done / current / pending lamp on each, the three probes with what each one reports, and Export backup, Engine log and Console at the foot. The step, the probes and what commissioning is moved to the bar along the bottom of the window. Each step is now a backlit page at the height that step needs, not a panel you scroll: what the step is and the rules that govern it on the left with the key that does it, and what the step records on the right — for Publish, the hardware profile, the lighting bridge, the audio console, the control surface, the Companion profile, whether an override is recorded, the four steps above it as done and the support archives. Publish now says **Publish with override…** before you press it when a probe is not green, instead of looking the same and then asking. Support is always on the right — the theme and the interface size (which used to be reachable only from the command palette), the backups, the diagnostics a support ticket needs, the sample data that asks first, the versions, and Restart the bridge as the one red command — and it stays there whichever mode the middle is in. On a smaller screen the Support panel gives the page its width back and the step still fits without scrolling. The screens you see before the app is ready use the same shape: a cold boot says what it is waiting for, and a failed start says what failed in plain words with the raw code small beneath it, Retry startup on the state itself, and every band naming what to do next.

- **Planning says how the day is going, and the timeline is a screen (visual overhaul A, Slice 6).** The toolbar is gone. Down the left, in the same place the Console and Lighting keep theirs, the day now says what it is: **ON TIME** or **SLIPPED**, with the date, how many tasks are scheduled and when the day runs, and under it the counts you scan for — what has slipped, what is blocked and which project it is, what is done, how many timers are running and what has not been scheduled at all. Under that: the timers running right now, each with its project, when it started, how long it has run and a Stop key; Timeline or Board; the day, with a key either side of it; the five projects that own the lanes; what the timers have put on each project today with the Time report beside it; and Board view and Backup at the foot. The day, what is scheduled, what is not, what is done and the time tracked moved to the bar along the bottom of the window. The timeline is now a backlit screen with the filters and the search on its own header: one lane per project, a card sitting at each task's start with its duration drawn as a bar beside it, a second row when two cards would cover each other, a card near the right edge hanging to the left of its start so it stays on screen, every other hour labelled when the hours are close together, the now marker running the full height, and the unscheduled tray under it. The panel on the right is always there and shows the task you picked — its schedule, its checklist, its notes, the project's other tasks, what happened today and Delete task — instead of a dialog you had to open and close; Escape takes the task off it. Running a timer, stopping one and deleting a task now reach the engine, which has always answered those requests but was never asked.

- **The lighting panel shows the whole selected fixture at once — no more tabs (visual overhaul A, Slice 5b).** The Scene · Fixture · Palettes tabs are gone. The panel shows whatever you have selected, with all of its parts on one page: what the fixture is, Identify and on/off, its intensity and colour temperature, where it stands in the room, the DMX address it answers on and whether anything overlaps it, what the saved scene holds for it beside what the rig is doing right now, and Delete fixture at the end. Inspecting a group from the rail now shows the group straight away instead of asking you to click a tab, and picking a fixture on the plot hands the panel back to that fixture. The palettes sit under whatever is selected, and their small icon buttons are proper targets now.

- **Lighting says what the rig is doing in one place, and the plot is a screen (visual overhaul A, Slice 5).** The toolbar, the bridge banner and the preview banner are gone. Down the left, in the same place the Console keeps its own, the rig now says what it is: whether the bridge is answering, whether the rig still matches the scene you recalled, or whether you are editing offline — each with a sentence in plain words and the way out beside it (open Setup for the bridge probe; save the changes or recall the scene again; save the preview to the rig or discard it). Under that: Lighting on, Cut all, the grand master, the search with the scenes you recalled most recently, the scenes, the groups, and the rig's standing actions — Add fixture, Patch, Preview, DMX monitor, Highlight, Solo and Find, which no longer hide in an overflow menu on smaller screens. The bridge, the universe, how many fixtures are patched and whether the scene is saved moved to the bar along the bottom of the window. The stage plot is now a backlit screen: it carries a blue edge while you edit offline, and says "locked · bridge unreachable" on its face when nothing you press will reach the rig.

- **The panel on the right shows the whole selected channel at once — no more tabs (visual overhaul A, Slice 4c).** The Preamp / EQ / Dyn / Routing tabs are gone. The panel now reads top to bottom as one page: what the source is, the send to the mix you are on and every other mix it feeds with their pre-fader and mute settings, the equaliser, the compressor and the gate, the meter with Peak hold and Reset peaks beside its readouts, and a Channel section that prints the flags the desk reports — stereo link, whether sends are pre- or post-fader, and where the compressor and gate stand — which were not visible anywhere before. The keys that used to switch tabs (P, E, D, R) now take you to that part of the panel. Selecting an output shows that mix instead. The equaliser's frequency and gain scales are legible now: they were printed at 9.5 px in a grey that failed contrast in every theme and was nearly invisible on the light one.

- **Every channel strip reads in one order, and 48 V is on the strip (visual overhaul A, Slice 4b).** A strip now goes: the name and what the desk says it is, the level it is set to, what feeds it, the Mute and Solo keys, then the fader with its scale and its meter. Everything you read or press during a take sits within a glance of the name, and only the fader grows with the window. 48 V has moved out of the menus onto the strip as a key that lights when it is on and asks twice before it changes; the preamp gain sits beside it and opens a number when you press it, with the arrows nudging a decibel at a time (riding the gain by hand is still the knob in the inspector). Playback strips show what they are sending to the cue mixes without being selected, and output strips show dim and mono as lamps. When the console is locked, every tier now says so where your hand is — "locked · run the audio probe" — and each refused control is outlined and says why. When nothing is metering, the meters are empty rather than pretending.

- **The Console says how it is, what it is doing and what you can do about it in one place on the left (visual overhaul A, Slice 4a).** The top bar, the monitor bar, the health bar, the snapshot deck and the three warning bands across the mixer are gone. Their controls and facts now sit in a fixed column beside the mixer: the console's state and the sentence that explains it, the way out of that state, a solo or clip latch you can see from anywhere, Talkback, Dim, Mono, the mix target, the main level with its meter and dB scale, the eight snapshot slots with the name you gave them and when each was last recalled, and Sync from TotalMix, Run audio probe and Clear clips at the foot. The console link, the metering source, the last sync and the bank moved to the bar along the bottom of the window, which every workspace now shares. A fault code no longer leads the sentence: the console says what happened, and the code stands beside it for the engine log.

- **One family of keys, lamps, wells and state displays carries every state in the program; lit keys bloom, locked keys are outlined and say why (visual overhaul A, Slice 3).** The design system gains the state display, the key (command, primary, danger, toggle, momentary, arm-then-apply with its countdown, 48 V hazard, segmented), lamps with their words, latches, readouts, sliders, the fader groove, the meter, fields, screens, the plate sections, the drawer and a still loading state. Dialogs, the command palette and toasts float as one plate over the surface; buttons are keys with sentence-case labels that no longer jump on hover; status badges speak the same words as the state displays. The primitives are built in all three themes on Storybook pages that the UI contract measures.

- **One header on every surface, including Setup / Support and the recovery screens; the subsystem lamps show the same severity the workspace shows (visual overhaul A, Slice 2).** The shell is one grid — header, cluster, bay, plate, footer — on every surface: the crest and the product with its owner's name, the four workspace tabs with their shortcuts, the Lighting / Audio / Surface lamps, the Solo and Scene drift latches and the clock. Setup / Support and the startup and recovery screens render inside it with the tabs locked until there is somewhere to go. A subsystem lamp now mirrors the worst state its workspace shows, so a failed action or an unreachable bridge is red in the header too.

- **Keyboard shortcut labels follow the host OS (2026-09 audit remediation, Slice 12 — operator decision 8).** Every shortcut hint — the workspace footers, the command palette, the `?` overlay, tooltips, toasts and aria labels — used to show the Mac glyphs (⌘ ⌥ ⇧) with "(Ctrl+K on Windows)" footnotes. A shared `shortcutGlyphs` helper now renders `Ctrl+K`, `Alt+S`, `Ctrl+Shift+M` on Windows and Linux and keeps `⌘K`, `⌥S`, `⌘⇧M` on macOS; the footnotes are gone.
- **DMX values read as decimal (2026-09 audit remediation, Slice 12 — operator decision 8).** The full DMX monitor and the inspector's channel list show 0–255, the way every lighting desk and fixture menu does, instead of two-digit hex.
...
- **`npm run scripts:test` runs the script tests on Windows (2026-09 production readiness, Slice 1).** The lane's glob was single-quoted, which `cmd.exe` hands to Node literally, so the thirteen `scripts/**/*.test.mjs` files ran under CI's bash but never on the workstation, where the lane reported zero tests. The glob is double-quoted and the same tests run on both.
- **The workspace qualification lane publishes through the real probe gate (2026-09 production readiness, Slice 1).** `scripts/tauri-workspace-qualification.mjs` published `stage: ready` without running the control-surface probe and without the explicit probe override, so the engine's publish gate (2026-09 audit remediation, Slice 8) refused it wherever the lane ran, and the CI `qualification` job was red at that step. The lane now runs all three probes and sends `overrideProbes: true` only when a probe cannot pass on the host: no TotalMix (`SSE_TAURI_QUALIFICATION_SKIP_AUDIO_PROBE=1`, CI) or port 80 not bindable for the lighting probe server.
- **Bone and Graphite are legible on every surface, and the Console stops hiding the Phones levels (visual overhaul A, Slice 10).** Walking all 27 screens in all three themes with the pixel-sampled reader turned up 253 pieces of text below the 4.5:1 the system asks for; there are none left. The worst of them was not a colour at all: on the three states where the Console is locked, the Phones 1 strip was sliced down the middle and Phones 2 was hidden altogether, so their levels could not be read. The mixer was 32 px wider than the bay it sits in, and a tier that carried a lock note in its heading stole 170 px from the Outputs tier, which had been written to take whatever was left rather than to keep its own three strips. All thirteen strips now fit the bay exactly, on every state, and the Outputs strips keep their width whatever the heading beside them says. The rest was ink: the small labels on the Lighting panel, the scene tiles' second lines, the group chips, the Console's captions and its send cards were all a shade too pale for the surface they sat on — worst on Bone, where a fixture's own label read at 1.7:1 against the plot. Fixture names and their readings now sit on a small chip of the plot floor, so they stay readable when they fall across a lit beam instead of only over bare floor, and the meter scale's numerals have room around them.

- **Operator-facing copy no longer speaks engine (2026-09 audit remediation, Slice 12 — operator decision 8).** Engine refusals ("Channel 'X' is not part of this console", "Channel 'X' has no 48V switch — only the front preamp inputs do"), action summaries ("Sent 3 changes for 'Host' to TotalMix — waiting for the console to confirm"), the audio status line and probe result (TotalMix menu paths and its own port names instead of "three OSC remote slots"), the Console's OSC-disabled / stale / offline / assumed banners, the playback inspector card and the engine-restart dialog are reworded for the person at the desk. Cut all names the preview buffer when preview mode is active.
- **Bone chrome header and the Console's smallest type are legible again (2026-09 audit remediation, Slice 10).** The shell header's gradient ended in a Studio-only literal (`#060706`) that every theme inherited, so on Bone the header was a near-black band with dark navigation labels on it; the header's action-chip highlights were cream literals too. Both are theme tokens now (`--color-shell-header-bottom`, `--color-shell-action-glass-*`), Bone gets a light header and dark-alpha highlights, and a design-system test fails if a colour literal returns to that stylesheet. In the Console, about forty captions sat at 8, 8.5 or 9 px and many of them were painted with the 22 %-alpha hairline colour (`--fg-4`, roughly 1.6:1), unreadable on the studio monitor. Every one now reads the new `--audio-type-micro` token (9.5 px) and paints `--audio-caption-muted`, which on Studio and Graphite is `--fg-3` raised from 0.50 to 0.56 alpha (about 5:1); the shared HealthBar footer caption moves from a 9 px literal to the same floor. A new `audio-legibility.spec.ts` guards it at 1920 and 2560 in Studio and Bone: no rendered Console text below 9.5 px, no text painted `--fg-4`, every visible text run at least 3:1 against its effective background, and the workspace navigation labels at least 4.5:1 against both ends of the header gradient in all three themes.
