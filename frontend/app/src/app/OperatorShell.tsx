...
import { AppShellFrame, Button, MetricCard, StatusBadge, Surface, type StatusTone } from "@sse/design-system";
import {
  useShellSnapshot,
  type JsonValue,
  type ShellState,
...
type ShellExperience = "ready" | "recovery" | "startup";
...
function deriveShellExperience(shellState: ShellState): ShellExperience {
...
function buildStartupSteps(lifecycle: ShellState["lifecycle"]): StartupStep[] {
  const stages = [
    "launching-process",
...
function getFailureTitle(startupFailure: StartupFailure | null) {
...
function formatPathLabel(key: string) {
  switch (key) {
    case "appDataDir":
      return "App data";
    case "backupDir":
...
function feedbackBadgeTone(tone: FeedbackTone): StatusTone {
  if (tone === "ok") {
    return "healthy";
  }
