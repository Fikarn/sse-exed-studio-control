...
function shapeForMounting(mounting: FixtureMounting): ReactElement {
  switch (mounting) {
    case "grid-panel":
      return <rect x={-9} y={-9} width={18} height={18} rx={2} fill={SHELL_FILL} stroke={SHELL_STROKE} strokeWidth={1} />;
    case "grid-soft":
      return <rect x={-13} y={-9} width={26} height={18} rx={4} fill={SHELL_FILL} stroke={SHELL_STROKE} strokeWidth={1} />;
    case "wall-bar":
      return <rect x={-22} y={-3} width={44} height={6} rx={1} fill={SHELL_FILL} stroke={SHELL_STROKE} strokeWidth={1} />;
    case "stand":
    default:
...
export function FixtureMarker({
  ...
  return (
    ...
    >
      ...
      {selected ? (
        <circle r={mounting === "wall-bar" ? 26 : 14} fill="none" stroke={SELECTED_STROKE} strokeWidth={1.5} strokeDasharray="4 3" />
      ) : null}
    </g>
  );
}
