...
function useSliderInteraction(
  ...
  const { value, onChange, onCommit, step = 0.01, locked } = props;
  ...
  const onPointerDown = useCallback(
    (event: PointerEvent<HTMLElement>) => {
      ...
      const next = clamp01(fractionFromPointer(event.currentTarget, event));
      latest.current = next;
      onChange?.(next);
    },
    [fractionFromPointer, locked, onChange]
  );
  ...
  const onKeyDown = useCallback(
    ...
    [locked, onChange, onCommit, step, value]
  );
  return { onPointerDown, onPointerMove, onPointerUp, onKeyDown };
}

function ariaProps(props: SliderBaseProps) {
  return {
