...
| `rust`               | `rust:fmt:check`, `rust:clippy`, `native:check`, `native:test`, `native:acceptance` with `SSE_AUDIO_SIMULATED_INPUT_MODE=1` (the audio probe passes in simulated input mode, so the harness reaches the `ready` gate on a host without TotalMix) and `SSE_NATIVE_ACCEPTANCE_SKIP_AUDIO_SYNC=1` (skips only the `audio.sync` / `audio.snapshot.recall` assertions, which need a real console) |
