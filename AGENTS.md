...
- **Operator ruling, absolute since 2026-09-26: Studio Control always runs at `2560×1440`, fullscreen on an external display, on a Windows machine.** Any other resolution and any other operating system is not to be considered — "unnecessary and a waste of time". Do not design, fix, test, verify or document anything for another size (1920×1080, 1280×800, a windowed size, a laptop panel) or another system (Linux, macOS). What existed only for them — captures, tests, guards, fallback layouts, Studio Preview, the windowed layout, CI steps, macOS paths, docs — was removed by the new pages program's Slice SW (D22); do not bring any of it back. CI runs on Linux runners as infrastructure, but nothing depends on Linux rendering: it compares no captures and samples no contrast. (This makes the rulings of 2026-09-07, visual overhaul A's D4, and 2026-09-18 absolute; the 2026-09-18 "leave the guards and ask before deleting one" and the 2026-09-23 linux-capture duty ended with Slice SW.)
...
- `npm run dev:check` — full local code-health bundle, its steps side by side at below-normal priority (`scripts/dev-check.mjs`, about 20 s on the studio workstation): protocol, format, lint, script tests, file health, rustfmt, clippy, frontend typecheck, the frontend tests with their coverage floors, native tests. Each step logs to `node_modules/.cache/dev-check/`; a failed step's log is printed (whole when short, else its failure lines and its end). `npm run dev:check:serial` runs the same steps one after another at the normal priority: re-run a timing failure there before believing it (`DEV_CHECK_PRIORITY=normal` keeps `dev:check` at the normal priority too). (`native:check` left the bundle on 2026-09-25: `rust:clippy --workspace --all-targets` checks more.)
...
- `node scripts/ui-census.mjs` (from `frontend/app`) — measure all 63 UI-contract boards; `--write-ratchets` re-seeds `frontend/app/tests/ui-contract.ratchets.json`. Takes ~4 minutes. See `docs/DEVELOPMENT.md §2c`.
...
- `npm run native:sbom:win:write` — CycloneDX SBOMs of what the packaged bundle carries, under `release/sbom/windows/` (needs `cargo-cyclonedx` and `npm run native:sbom:tools:install`). The `release-evidence` workflow runs them; see `docs/RELEASE.md §Release Evidence`.
- `npm run native:release:win:local` — the target-host shipping release gate when QtIFW tools are installed.
...
3. inspect the result on the fixed studio monitor when human fit judgment matters.
...
- Frontend Playwright + visual baselines: `npm run frontend:playwright:test`. `visual-review.spec.ts` commits `toHaveScreenshot` baselines under `frontend/app/tests/__visual__/visual-review.spec.ts-snapshots/` (`*-win32.png` at `2560×1440` only, compared on the studio workstation before each push; Playwright ignores captures on any other system, so the CI `frontend-e2e` job, on Linux, compares none and uploads the Playwright report and traces as artifacts). `storybook.spec.ts` does the same for the Storybook static build and is skipped off Windows. Both the behaviour specs and the visual lanes run against the built app (Playwright's `webServer` is `vite preview` over `frontend/app/dist`, reused when already running; the Storybook lane reads `storybook-static`), so after any frontend source edit run `npm run build --workspace frontend/app` (and `npm run frontend:storybook:build` for the Storybook lane) before `npx playwright test …`, or the specs exercise the previous build.
- UI contract: `frontend/app/tests/ui-contract.spec.ts` measures 63 boards (21 fixtures × 3 themes at 2560×1440) plus the Storybook primitive pages against system §10, ratcheted per board in `frontend/app/tests/ui-contract.ratchets.json` — a measure may fall, never rise. It samples contrast on Windows only (the workstation's run before each push; CI samples none). Three sibling gates run outside it: `scripts/check-operator-copy.mjs`, the design system's `css-literals.test.ts` allowlist, and the tokens package's `themes.contrast.test.ts`. `docs/DEVELOPMENT.md §2c` has the commands, the re-seed flow and the traps.
...
- Target-host lanes: Windows native verification is the blocking release gate; there is no other target host (Slice SW removed the macOS one).
...
| `frontend-e2e`       | The quarantine list's check, then `frontend:playwright:test:blocking` — Playwright's `default` project on the Linux runner: every behaviour spec and the UI contract, with no screenshots compared and no contrast sampled (the studio workstation verifies both, on Windows, before each push) — which is what the job fails on, then `frontend:playwright:test:quarantine` in a step that reports and never fails the job (the wall-clock cases named in `frontend/app/tests/quarantine.json` — none since production readiness S15). Uploads `playwright-report`, `playwright-test-results` (traces) and `playwright-quarantine` artifacts. |
...
| `qualification`      | `tauri:setup-support:qualify` + `tauri:workspaces:qualify` under `xvfb`, with `SSE_TAURI_QUALIFICATION_TIMEOUT_MS=180000`, `LIBGL_ALWAYS_SOFTWARE=1`, and `SSE_TAURI_QUALIFICATION_SKIP_AUDIO_PROBE=1` (skips the audio block; since Slice 2b the lanes' shells run the simulated console, so no TotalMix is involved either way). Starts with the other jobs and keeps a Rust cache of its own (the dependencies of the debug `test-bridge` shell and the engine the lanes build; the workspace's own crates still compile). Uploads `tauri-qualification-evidence` artifact.                                                                 |
...
A second workflow, `.github/workflows/release-evidence.yml`, runs on a `v*` tag or by hand: a clean Windows runner builds the packaged bundle, its SHA256 manifest and its CycloneDX SBOMs and keeps them as run artifacts. It publishes nothing, builds no installer, and its signing step is dormant until certificate secrets exist.

These jobs are required merge hygiene on `main`. They are **not** the release gate — `npm run native:release:win:local` and `npm run release:verify` on the operator workstation remain the release acceptance mechanism.

## Release posture

- A QtIFW offline installer for Windows, plus QtIFW maintenance-tool update-repository archives.
...
- Deployment profile: one fixed studio workstation, unsigned controlled deployment. Public signing (a Windows certificate) is deferred — see `docs/PRODUCTIZATION_PLAN.md §3`.
...
- Keep ignored local outputs out of commits: `node_modules/`, `release/`, `artifacts/`, `.tools/`, `native/**/target/`, `native/**/build/`, `frontend/**/dist/`, `frontend/**/storybook-static/`, Playwright reports, and test results.
...
- operator-visible changes have Tauri visual review evidence and, when needed, human inspection on the fixed `2560×1440` studio monitor;
