...
pub(super) fn load_lighting_cues(
    settings: &HashMap<String, String>,
) -> Vec<LightingEditorCueState> {
...
pub(super) fn resequence_cues(cues: &mut [LightingEditorCueState]) {
    for (index, cue) in cues.iter_mut().enumerate() {
        cue.ordinal = (index as i64) + 1;
    }
}

pub(super) fn serialize_lighting_cues(
...
pub(super) fn lighting_cue_snapshot_from_state(
...
pub(super) fn next_custom_cue_id(cues: &[LightingEditorCueState]) -> String {
    let next_index = cues
        .iter()
        .filter_map(|cue| {
            cue.id
