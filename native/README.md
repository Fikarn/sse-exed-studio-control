...
- when the saved data holds no completed setup and no earlier import, the engine will auto-import a legacy `db.json` from `SSE_LEGACY_DB_PATH` or from `<app-data>/import/db.json`; the working directory is never scanned. Since the new pages program's Slice 2 the import writes only whether setup is complete and the page to open (Lighting for a `dashboardView` of `lighting`, the Console otherwise) and ignores the file's Planning data; Slice 2b retires it
- the saved data is at schema 8 since that slice: the 7 → 8 step drops Planning's four tables and every `planning.*` setting and turns a saved Planning page into the Console, after the verified `pre-migration` copy every upgrade writes; older builds refuse schema 8
