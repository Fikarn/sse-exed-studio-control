...
- the engine imports no legacy `db.json` since the new pages program's Slice 2b retired the import (`storage.importLegacyDb` and the start-up auto-import): a start that finds `SSE_LEGACY_DB_PATH` set, or a file at `<app-data>/import/db.json`, writes one `WARN` line naming it ("A db.json at <path> was left alone: Studio Control no longer imports db.json files.") and reads nothing, and Verify and Restore refuse a `db.json` by name before anything is written. `SSE_DISABLE_AUTO_IMPORT` is no longer read
